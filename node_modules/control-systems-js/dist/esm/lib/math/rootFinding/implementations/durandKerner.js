import { abs, add, complex, divide, multiply, subtract } from '../../complex';
...
export class DurandKerner {
    constructor() {
        ...
        this.toMonicForm = (coefficients) => {
            ...
            if (coefficients[0] && !(((_a = coefficients[0]) === null || _a === void 0 ? void 0 : _a.re) === 1 && ((_b = coefficients[0]) === null || _b === void 0 ? void 0 : _b.im) === 0)) {
                monicCoefficients = coefficients.map((coefficient) => divide(coefficient, coefficients[0]));
            }
    ...
    evalPolynomial(coefficients, value) {
        ...
        const result = coefficients.slice(1).reduce((prev, curr) => {
            return add(multiply(prev, value), curr);
        }, coefficients[0]);
        return result;
    }
    /**
    ...
    generateInitialRootGuess(polynomialOrder, initialResult) {
        ...
        for (let i = 1; i < polynomialOrder - 1; i++) {
            initialGuess[i] = multiply(initialGuess[i - 1], initialResult);
        }
        return initialGuess;
    }
    /**
        ...
        while (true) {
            for (let i = 0; i < a0.length; i++) {
                ...
                for (let j = 0; j < a0.length; j++) {
                    if (i !== j) {
                        result = multiply(subtract(a0[i], a0[j]), result);
                    }
                }
                // fixed point iteration
                // x_(n+1) = x_n - f(x_n)/(x_n - q)(x_n-r)(x_n - p)...
                a1[i] = subtract(a0[i], divide(this.evalPolynomial(coefficients, a0[i]), result));
            }
            iterCount += 1;
