...
export class PolynomialOperations {
    ...
    add(pol1, pol2) {
        ...
        for (const index of range(polynomialHighestOrder)) {
            const coefficient = add(orderedPol1[index] || 0, orderedPol2[index] || 0);
    ...
    subtract(pol1, pol2) {
        ...
        const resultPolynomial = [];
        for (const index of range(polynomialHighestOrder)) {
            const coefficient = subtract(orderedPol1[index] || 0, orderedPol2[index] || 0);
            resultPolynomial.push(coefficient);
        }
        return this.reversePolynomial(resultPolynomial);
    }
    multiply(pol1, pol2) {
        ...
        for (const i of range(polSize1)) {
            for (const j of range(polSize2)) {
                result[i + j] = add(result[i + j], multiply(orderedPol1[i], orderedPol2[j]));
            }
        }
